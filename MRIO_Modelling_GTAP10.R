...
setwd("T:/MSA/papers_internal/work_in_progress/Mi_Homogenized_Datainfrastructure/0_Data/2_IO Data/GTAP_10_MRIO/")
...
Countries <- read_csv("Countries_Overview.csv")%>%
  mutate(Country_Name = ifelse(Name == "tjk", "Tajikistan", Country_Name))
...
CO2_0 <- np$load("CO2_Gtap_10_2014_re.npy")
Y_0   <- np$load("gtap10_2014_re_y.npy")
Z_0   <- np$load("gtap10_2014_re_z.npy")
...
Y_R1 <- array_reshape(Y_R, c(9165, 141))%>%
  as.data.frame()
Z_R1 <- array_reshape(Z_R, c(9165, 9165))%>%
  as.data.frame()
...
help <- data.frame('A' = rep(1:65,141), 'B' = 1:9165)%>%
...
Y_R2.1 <-Y_R2 %>%
  mutate(Y_total = select(., Country_1:Country_141) %>% rowSums(na.rm = TRUE))%>%
  select(Country_A, Sector, Y_total)

Z_R2.1 <- Z_R2 %>%
  mutate(Z_total = select(., '1_1':'141_65') %>% rowSums(na.rm = TRUE))%>%
...
A_0 <- Z_R2 %>%
  pivot_longer(c(1:9165), names_to = c("Country_B", "Sector_B"), names_sep = "_", values_to = 'Z')%>%
  left_join(OUT, by = c("Sector_B" = "Sector_A", "Country_B" = "Country_A"))%>%
  mutate(Z=Z/OUT)

# Stupid Coding, but pivot_wider does not work easily

A_1 <- expand_grid("Country_A" = 1:141, "Sector_A" = 1:65)
...
transport_national_carbon_intensities   <- read_csv("Transport_National_Carbon_Intensities_L.csv")
electricity_national_carbon_intensities <- read_csv("Electricity_National_Carbon_Intensities_L.csv")

national_non_carbon_intensities         <- read_csv("National_NON_Carbon_Intensities_L.csv")
gas_national_carbon_intensities         <- read_csv("Gas_National_Carbon_Intensities_L.csv")
...
# 
# national_carbon_intensities <- bind_rows(national_carbon_intensities, L_2_2)
# 
# }
# 
# write_csv(national_carbon_intensities, "National_Carbon_Intensities_L.csv")
...
# write_csv(transport_national_carbon_intensities, "Transport_National_Carbon_Intensities_L.csv")
...
# }
# 
# write_csv(electricity_national_carbon_intensities, "Electricity_National_Carbon_Intensities_L.csv")
...
# ##   mutate(CO2 = ifelse(Country_A == i, CO2, 0))
# 
# CO2_OUT_6 <-   left_join(CO2_R6, OUT)%>%
#   mutate(CO2_OUT = ifelse(OUT != 0, CO2/OUT, 0))%>%
#   select(-CO2, - OUT)
# 
# L_2_6_1 <- L_2_6 %>%
...
# }
# 
# write_csv(gas_national_carbon_intensities, "Gas_National_Carbon_Intensities_L.csv")
...
cons_tot <- left_join(cons_dom_1, cons_int_1)%>%
...
rm(EE_1, EE_2, EE_3, EE_4, EE_5, EE_6, EE_7, EE_8, EE_9, EE_10, EE_11)
...
df_final <- expand.grid(GTAP = c(1:65))%>%
  ...
  left_join(cons_tot_0, by = c("GTAP" = "Sector"))%>%
  left_join(CO2_dir, by = c("GTAP" = "GTAP_0"))%>%
  ...
  rename(Total_HH_Consumption_MUSD = Cons_Tot, Electricity_MUSD = ELY_0, Electricity_GWh = ELY_per_sec)%>%
...
df_final_out <- df_final %>%
  select(GTAP, CO2_Mt, CO2_Mt_within, CO2_Mt_Electricity, CO2_Mt_Transport, Electricity_MUSD, Electricity_GWh,
...
names(list_0) <- Countries$Country_Name[1:length(list_0)]

write.xlsx(list_0, "Carbon_Intensities_Full_All_incl_Gas_Coal_PC_direct.xlsx", append = TRUE, colNames = TRUE)
