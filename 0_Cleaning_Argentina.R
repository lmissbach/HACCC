...
path_0 <- "../0_Data/1_Household Data/3_Argentina/1_Data_Raw/Data"
...
cooking.code <- distinct(hogares_1, cooking_fuel)%>%
  ...
  mutate(Cooking_Fuel = c("Mains gas?", "Bulk gas (zeppelin)?", "Gas in tube?", "Gas in a carafe?", "electricity?", "Kerosene / firewood / charcoal?", "other?" ))
heating.code <- distinct(hogares_1, heating_fuel)%>%
  arrange(heating_fuel)%>%
  mutate(Heating_Fuel = c("Mains gas?", "Bulk gas (zeppelin)?", "Gas in tube?", "Gas in a carafe?", "electricity?", "Kerosene / firewood / charcoal?", "other?" ))
...
occupation.code <- distinct(hogares_1, ind_hhh)%>%
  arrange(ind_hhh)%>%
  mutate(Industry = c("Salaried","Own legal company account","Own account of non-legal company","Employer / employer / partner of legal society","Employer / partner partner of a non-legal company","Unpaid family employee", NA))
