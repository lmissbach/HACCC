...
appliances_1 <- data_0_viviendas %>%
  ...
  mutate(ac.01 = ifelse(ac.01 == 1,1,0))
...
appliances_3 <- left_join(appliances_2, appliances_1, by = "folioviv")%>%
  ...
  filter(hh_id %in% household_final$hh_id)

write_csv(appliances_3, "../0_Data/1_Household Data/3_Mexico/1_Data_Clean/appliances_0_1_Mexico.csv")

# Codes
Mex_District <- read_csv("../0_Data/1_Household Data/3_Mexico/2_Codes/District_Codes_Mexico_New.csv")%>%
  select(Nom_Ent, Cve_Ent, Nom_Mun, Cve_Mun)%>%
  distinct()

library(readxl)
Mex_District <- read_excel("T:/MSA/papers_internal/work_in_progress/Mi_Homogenized_Datainfrastructure/0_Data/1_Household Data/3_Mexico/2_Codes/District_Codes_Mexico_New.xlsx", 
                                        skip = 3)%>%
  select(NOM_ENT, CVE_ENT, NOM_MUN, CVE_MUN)
