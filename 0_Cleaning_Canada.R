...
household_information <- data_diary %>%
  rename(hh_id = CaseID, hh_weights = WeightD, province = Prov, hh_size = HHSize,
...
variable_names_di <- data.frame("Variable" = colnames(data_diary))%>%
  mutate(TypeA = "Diary")
variable_names_iv <- data.frame("Variable" = colnames(data_interview))%>%
  mutate(TypeB = "Interview")
variable_names <- full_join(variable_names_di, variable_names_iv, by = "Variable")

# write.xlsx(variable_names, "../0_Data/1_Household Data/3_Canada/9_Documentation/Variable.Names.Canada.xlsx")

households_A <- distinct(data_interview, CaseID)%>%
  arrange(CaseID)%>%
  mutate(Type_A = "Interview")

households_B <- distinct(data_diary, CaseID)%>%
  arrange(CaseID)%>%
  mutate(Type_B = "Diary")

households <- left_join(households_A, households_B)%>%
  rename(hh_id = CaseID)

expenditures_interview_0 <- data_interview %>%
  rename(hh_id = CaseID)%>%
  select(hh_id, CC001_C:TX001)%>%
  pivot_longer(-hh_id, names_to = "item_code", values_to = "expenditures_year")%>%
  arrange(hh_id, item_code)%>%
  mutate(Type = "Interview")

# Only households which are present in interview

expenditures_interview_1 <- expenditures_interview_0 %>%
  left_join(households)%>%
  filter(is.na(Type_B))

# Only households which are present in diary

expenditures_interview_2 <- expenditures_interview_0 %>%
  left_join(households)%>%
  filter(!is.na(Type_B))

# not required anymore because all the information is in the diary dataframe
# I tested it

rm(expenditures_interview_2, expenditures_interview_0, variable_names, variable_names_di, variable_names_iv, households_A, households_B)

# Observations should theoretically be identical with observations in diary

expenditures_diary <- data_diary %>%
  rename(hh_id = CaseID)%>%
  select(hh_id, CC001:TX001)%>%
  pivot_longer(-hh_id, names_to = "item_code", values_to = "expenditures_year")%>%
  arrange(hh_id, item_code)%>%
  mutate(Type = "Diary")

# 6 households have diverging numbers between interview and diary - 31 special observations --> later TBD
# Work with diary information for those households

Item.Codes.Diary <- distinct(expenditures_diary, item_code)%>%
  arrange(item_code)%>%
  mutate(Type_A = "Diary")

Item.Codes.Interview <- distinct(expenditures_interview_1, item_code)%>%
  arrange(item_code)%>%
  mutate(Type_B = "Interview")

Item.Names <- read.xlsx("../0_Data/1_Household Data/3_Canada/3_Matching_Tables/Old/Item_Code_Description_Canada.xlsx")%>%
  rename(item_code = code, item_name = description)

Item.Codes <- left_join(Item.Codes.Diary, Item.Codes.Interview)%>%
  full_join(Item.Names)%>%
  mutate(Category = NA)

# write.xlsx(Item.Codes, "../0_Data/1_Household Data/3_Canada/3_Matching_Tables/Item_Code_Description_Canada.xlsx")

# Item-Codes to be deleted

Item.Codes.Deleted <- read.xlsx("../0_Data/1_Household Data/3_Canada/3_Matching_Tables/Item_Categories_Concordance_Canada.xlsx", colNames = FALSE)%>%
  pivot_longer(-X1, names_to = "drop", values_to = "item_code")%>%
  select(-drop)%>%
  filter(!is.na(item_code))

# Final dataframe with all expenditures for diary households

expenditures_diary_1 <- expenditures_diary %>%
  left_join(Item.Codes.Deleted)%>%
  filter(is.na(X1))%>%
  select(-X1)%>%
  filter(expenditures_year > 0)

write_csv(expenditures_diary_1, "../0_Data/1_Household Data/3_Canada/1_Data_Clean/expenditures_items_Canada.csv")

# Decision 09. February 2023:
# We will not extrapolate expenditures to interview households
# We will only work with diary households and use respective weights
# Imputations to be discussed later

# Prior work by Paula Blechschmidt

#data_raw_interview <- data_interview
#data_raw_diary <- data_diary
#
