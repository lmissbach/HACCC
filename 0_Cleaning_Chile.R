...
data_chile_clean <- read_dta("../0_Data/1_Household Data/3_Chile/1_Data_Clean/LAC_Clean/CHL_EPF_2016-2017.dta")
...
data_1_personas <- data_0_personas %>%
  select(ZONA, FOLIO, FE, NPERSONAS, JHOGAR, PARENTESCO, SEXO, EDAD, EDUNIVEL, CAE, AECIUO88, TRANSFERENCIAS_RECIBIDAS, VP05D)%>%
  rename(province = ZONA, hh_id = FOLIO, hh_weights = FE, hh_size = NPERSONAS, 
         head = JHOGAR, family_status = PARENTESCO, sex_hhh = SEXO, 
         age_hhh = EDAD, 
         #marital_status = ECIVIL,
         edu_hhh = EDUNIVEL, 
         #edu_institution = EDUDEPENDENCIA,
         empl_hhh = CAE,
...
data_2_personas <- data_1_personas %>%
  filter(head==1)%>%
  select(hh_id, hh_size, hh_weights, province, sex_hhh, adults, children, age_hhh, edu_hhh, ind_hhh)%>%
  left_join(income)%>%
