...
kbl(mutate_all(LAC_Electricity_2, linebreak), format = "latex", caption = "Electricity Generation in 16 Countries of Latin America and the Caribbean", 
  ...
  footnote(general = "This table provides summary statistics for electricity generation in 16 different countries of Latin America and the Caribbean. It reports the share of electricity generated by each source in each country in 2020 [%] as well as the total annual electricity consumption [TWh] and per capita [Mwh]. Source: IEA (2021) and Our World in Data (Ritchie et al. 2020) for Barbados. Annual electricity consumption for Peru refers to 2019. ", threeparttable = T)%>%
...
for(i in Country.Set){
  ...
  etable(model_2.1.1.1, dict = dict_latex, tex = TRUE, file = sprintf("../1_Carbon_Pricing_Incidence/3_Analyses/1_LAC_2021/6_App/Latin-America-Paper/Tables/Table_2_OLS/Table_OLS_%s.tex", i),
         digits = 3, replace = TRUE, fitstat = c("n", "r2"), style.tex = tex.style, se.row = TRUE,
         title = sprintf("OLS-Regression Coefficients for %s", i),  label = sprintf("tab:OLS_%s",i), adjustbox = "width = 1\\textwidth, max height = 0.6\\textheight, center", placement = "htbp!")
