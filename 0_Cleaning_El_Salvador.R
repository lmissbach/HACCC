...
Gender.Code <- stack(attr(data_013$sex_hhh, 'labels'))%>%
  ...
  write_csv(., "../0_Data/1_Household Data/3_El Salvador/2_Codes/Gender.Code.csv")
...
Education.Code <- expand_grid(Education.Code.A, Education.Code.B)%>%
  ...
  write_csv(., "../0_Data/1_Household Data/3_El Salvador/2_Codes/Education.Code.csv")
Industry.Code <- distinct(data_013, ind_hhh)%>%
  arrange(ind_hhh)%>%
  mutate(ind_hhh = as.character(ind_hhh))
CIIU <- read.xlsx("../0_Data/1_Household Data/3_El Salvador/9_Documentation/Industry_Codes_CIIU_Rev_4.xlsx")
CIIU.2 <- CIIU %>%
...
Industry.Code.0 <- left_join(Industry.Code, CIIU, by = c("ind_hhh" = "Código"))%>%
  ...
  write_csv(., "../0_Data/1_Household Data/3_El Salvador/2_Codes/Industry.Code.csv")

Province.Code <- stack(attr(data_01$province, 'labels'))%>%
  rename(Province = ind, province = values)%>%
  write_csv(., "../0_Data/1_Household Data/3_El Salvador/2_Codes/Province.Code.csv")
District.Code <- stack(attr(data_01$district, 'labels'))%>%
  rename(District = ind, district = values)%>%
  write_csv(., "../0_Data/1_Household Data/3_El Salvador/2_Codes/District.Code.csv")
Lighting.Code <- stack(attr(data_01$lighting_fuel, 'labels'))%>%
  rename(Lighting_Fuel = ind, lighting_fuel = values)%>%
  write_csv(., "../0_Data/1_Household Data/3_El Salvador/2_Codes/Lighting.Code.csv")
Cooking.Code <- stack(attr(data_01$cooking_fuel, 'labels'))%>%
  rename(Cooking_Fuel = ind, cooking_fuel = values)%>%
  write_csv(., "../0_Data/1_Household Data/3_El Salvador/2_Codes/Cooking.Code.csv")
Water.Code <- count(data_01, water)%>%
  ...
  write_csv(., "../0_Data/1_Household Data/3_El Salvador/2_Codes/Water.Code.csv")
Toilet.Code <- stack(attr(data_0$r319, 'labels'))%>%
  rename(Toilet = ind, toilet = values)%>%
  write_csv(., "../0_Data/1_Household Data/3_El Salvador/2_Codes/Toilet.Code.csv")
