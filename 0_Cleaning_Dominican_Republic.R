...
data_0_clean <- read_dta("../0_Data/1_Household Data/3_Dominican_Republic/1_Data_Clean/LAC_Clean/DOM_ENIGH_2018.dta")%>%
  select(id_hogar, everything())
...
Province.Code <- distinct(data_0_socio, ID_PROVINCIA, DES_PROVINCIA)%>%
  ...
  rename(province = ID_PROVINCIA, Province = DES_PROVINCIA)%>%
  write_csv(., "../0_Data/1_Household Data/3_Dominican Republic/2_Codes/Province.Code.csv")

Municipality.Code <- distinct(data_0_socio, ID_MUNICIPIO, DES_MUNICIPIO)%>%
  arrange(ID_MUNICIPIO)%>%
  rename(municipality = ID_MUNICIPIO, Municipality = DES_MUNICIPIO)%>%
  write_csv(., "../0_Data/1_Household Data/3_Dominican Republic/2_Codes/Municipality.Code.csv")

District.Code <- distinct(data_0_socio, ID_DISTRITO_MUNICIPAL, DES_DISTRITO_MUNICIPAL)%>%
  arrange(ID_DISTRITO_MUNICIPAL)%>%
  rename(district = ID_DISTRITO_MUNICIPAL, District = DES_DISTRITO_MUNICIPAL)%>%
  write_csv(., "../0_Data/1_Household Data/3_Dominican Republic/2_Codes/District.Code.csv")
...
Lighting.Code <- distinct(data_12_socio, lighting_fuel)%>%
  ...
  bind_cols(Lighting_Fuel = c("Electricity (public)", "Electricity (private)", "Power Plant", "Kerosene", "Gas (Propan)", "Solar Panel", "Candle", "Others"))%>%
  write_csv(., "../0_Data/1_Household Data/3_Dominican Republic/2_Codes/Lighting.Code.csv")

Cooking.Code <- distinct(data_12_socio, cooking_fuel)%>%
  arrange(cooking_fuel)%>%
  bind_cols(Cooking_Fuel = c("LPG", "Kerosene", "Electricity", "Firewood", "Coal", "Does not cook", "Other"))%>%
  write_csv(., "../0_Data/1_Household Data/3_Dominican Republic/2_Codes/Cooking.Code.csv")
