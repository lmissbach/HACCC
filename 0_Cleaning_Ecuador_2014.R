...
data_02.1 <- data_02 %>%
  ...
  filter(PD04 == "Jefe")%>%
  select(hh_id, Education, Sex_hhh, ind_hhh, Ethnicity)
...
District.Code <- distinct(data_01, PROVINCIA)%>%
  ...
  mutate(district = 1:n())%>%
  write_csv(., "../0_Data/1_Household Data/3_Ecuador/2_Codes/District.Code.csv")

Village.Code <- distinct(data_01, CIUDAD)%>%
  arrange(CIUDAD)%>%
  rename(Village = CIUDAD)%>%
  mutate(village = 1:n())%>%
  write_csv(., "../0_Data/1_Household Data/3_Ecuador/2_Codes/Village.Code.csv")
...
Lighting.Code <- distinct(data_01, VI26)%>%
  ...
  write_csv(., "../0_Data/1_Household Data/3_Ecuador/2_Codes/Lighting.Code.csv")
...
Water.Code <- distinct(data_01, VI17)%>%
  ...
  write_csv(., "../0_Data/1_Household Data/3_Ecuador/2_Codes/Water.Code.csv")
...
Toilet.Code <- distinct(data_01, VI14)%>%
  ...
  write_csv(., "../0_Data/1_Household Data/3_Ecuador/2_Codes/Toilet.Code.csv")

Ethnicity.Code <- distinct(data_02, PD18)%>%
  arrange(PD18)%>%
  rename(Ethnicity = PD18)%>%
  mutate(ethnicity = 1:n())%>%
  write_csv(., "../0_Data/1_Household Data/3_Ecuador/2_Codes/Ethnicity.Code.csv")

Gender.Code <- distinct(data_02, SEXO)%>%
  arrange(SEXO)%>%
  rename(Sex_hhh = SEXO)%>%
  mutate(sex_hhh = 1:n())%>%
  write_csv(., "../0_Data/1_Household Data/3_Ecuador/2_Codes/Gender.Code.csv")
...
Education.Code <- distinct(data_02, PE47)%>%
  ...
  write_csv(., "../0_Data/1_Household Data/3_Ecuador/2_Codes/Education.Code.csv")
